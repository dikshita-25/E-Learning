...
# from ELearningProject.settings import *

client = razorpay.Client(auth=("rzp_test_6Q3JXjMaotC5cT","Wiq6uXhM32p3g85IGI4ZG9YV"))
...
def range(req):
    if req.method == "GET" :
        ...
        r1=req.POST["min"]
        r2=req.POST["max"]
        if r1 != "" and r2 != "" and r1 is not None and r2 is not None:
            queryset = Course.prod.get_price_range(r1,r2)
            context={}
            context['products'] = queryset
            return render(req,"my_course.html",context)
        else:
            return redirect("/")

def incpriceOrder(req):
    queryset = Course.objects.all().order_by('price')
    context={}
